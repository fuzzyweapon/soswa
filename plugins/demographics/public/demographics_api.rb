module AresMUSH
  module Demographics
    ...
    def self.name_and_alias(char)
      return nil unless char
      alias_format = Global.read_config("custom", "alias_format")

      unless char.alias.blank?
        if alias_format.blank?
          alias_format = "%{name} (%{alias})"
        end
        alias_format % {:name => char.name, :alias => char.alias}
      end
    end
